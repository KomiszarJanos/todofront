...
<template>
  <div class="container" v-if="!store.NotLogged">
   ...
   <div class="excelbutton" @click="store.Export"><button>Excel készítése</button></div>
  </div>
  <v-fade-transition>
  <ProjectForm v-if="!store.ShowProjects"></ProjectForm>
  </v-fade-transition>
</div>
<div v-else>
...
.searchcontainer {display: flex; justify-content: space-around; align-items: center; 
  margin-bottom: 34px; margin-top:34px; height: 20px;}
